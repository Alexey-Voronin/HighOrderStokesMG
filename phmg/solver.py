...
class StokesSolver(ABC):
    ...
    def __init__(
        ...
        if solver_type in ["lu", "none", "test"]:
    ...
    def solve(self, petsc_prof=False):
        ...
        info_dict = {
            "order": self.k,
            "nnz": self.solver.snes.ksp.pc.getOperators()[0].getInfo()["nz_allocated"],
            "ref": self.nref,
        ...
        # outfile.write(self.z.subfunctions[0], self.z.subfunctions[1], time=0)
    ...
    def get_parameters(self):
        outer_base = {
            ...
            # "petscpartitioner_parmetis_debug": "true",
            "snes_type": "ksponly",
            "snes_max_it": 1,
            ...
            "ksp_error_if_not_converged": False,  # deal with non-conv. in post-processing
        }
