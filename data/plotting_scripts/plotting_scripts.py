...
def get_frame(file_name):
    ...
    for name in ["nu", "Re", "udofs", "pdofs"]:  #'resids',  #'ncells',
        try:
            df.pop(name)
        except:
            pass
...
def plot_all(data, save_name=None, disc="TH", time_per_iter=True, warm=False):
    ...
    plot_field(data, vmax=None, fig_gs=(fig, gs), row=0)  # save_name='alfi_rel',
    plot_relatives(
        data, field_name=f"SNESSolve(time):{warm_str}", fig_gs=(fig, gs), row=1
    )
    if time_per_iter:
        plot_relatives(
            data,
            field_name=f"SNESSolve(time):{warm_str}",
            fig_gs=(fig, gs),
            row=2,
            per_iter_cost=True,
        )
...
def plot_field(
    ...
    all_values = np.concatenate(
        [
            df.pivot(index="order", columns="ref", values=field_name).values.ravel()
    ...
    if i == 0 and not fig_gs:
        fig.text(
            0.475, 0.01, "refinements", ha="center", va="center", fontsize=fontsize
        )
    if title:
