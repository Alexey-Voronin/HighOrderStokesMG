...
def plot_fraction(data_dict, save_name=None, with_setup=False, th_disc=True):
    ...
    set_figure(width=fs["height"] * (ncols * 1.0), height=1.0 * fs["height"])
    fig, axs_all = plt.subplots(nrows, ncols, sharey="row", sharex="col")
    # Font stuff
    plt.rcParams.update(
        {"text.usetex": True, "text.latex.preamble": r"\usepackage{amsfonts,amsmath}"}
    )
    text_font_size = 20
    number_font_size = 14
    ...
    for solver_name, data, ax in zip(
        data_dict.keys(), data_dict.values(), axs_all.ravel()
    ):
        ##########################################################################################
        ...
        rlx_type = (
            rlx_type_expected
            if rlx_type_expected in data.columns
            else f"PatchPC(times):Warm"
        )

        try:
            max_lvls = max(
                len(l) if not np.isnan(l).any() else 0 for l in data[rlx_type]
            )
        except:
            max_lvls = 0
        ...
        for (
            ...
            stacked_data.append(
                np.array(
                    setup_time_list
                    + [other_time, w_jacobian, grid_tranfer_time]
                    + rlx_time
                )
                / ref_time
            )
            ...
            col_table_data["T(rlx)/T(solve)"].append(
                np.sum(rlx_time) / col_table_data["T(solve)"][-1]
            )
        ...
        legends = ["other solve", "Jacobian", "grid transfer"] + [  #'setup',
            f"rlx($\ell={i}$)" for i in range(max_lvls)
        ][
            ::-1
        ]  # \ell=0 is the finest level!!!
        ...
        df_stacked = pd.DataFrame(
            stacked_data
        ).T  # Transpose to have each row represent a layer of the stack
    ...
    norm_scaled_bounds = (
        np.min(all_vals_scaled) * scale_down,
        np.max(all_vals_scaled) * scale_up,
    )
    ...
    norm_setup_over_total_bounds = (
        np.min(all_setup_over_total),
    ...
    norm_rlx_over_solve_bounds = (
        np.min(all_rlx_over_solve) * scale_down,
    ...
    def format_numbers(arr):
        def custom_format(x):
            if 0 < x < 1:
                return f"{x:.2f}".lstrip(
                    "0"
                )  # Strip leading '0' for numbers between 0 and 1
            elif x == 0:
    ...
    for j, ax in enumerate(axs_all):
        x_labels = [
            f"{o}"
            for o, r in zip(
                data_dict[list(data_dict.keys())[j]]["order"],
                data_dict[list(data_dict.keys())[j]]["ref"],
            )
        ]
        ...
        if j == 0:
            ...
            row_labels = [""] * len(
                table_row_labels
            )  # Keep the structure, but no visible labels
        ...
        norm_scaled = matplotlib.colors.TwoSlopeNorm(
            vmin=norm_scaled_bounds[0], vcenter=1, vmax=norm_scaled_bounds[1]
        )
        ...
        norm_scaled = Normalize(
            vmin=norm_setup_over_total_bounds[0], vmax=2.1
        )  # norm_setup_over_total_bounds[1])
        cmap_scaled = plt.get_cmap("binary")
        mappables_setup_over_total = ScalarMappable(norm=norm_scaled, cmap=cmap_scaled)

        norm_scaled = Normalize(
            vmin=norm_rlx_over_solve_bounds[0], vmax=norm_rlx_over_solve_bounds[1]
        )
        cmap_scaled = plt.get_cmap("cool")
        mappables_all_rlx_over_solve = ScalarMappable(
            norm=norm_scaled, cmap=cmap_scaled
        )
    ...
    # currently handles and labels are in a weird order.
    # The following code prescribed top-down label/bar order of the legend.
    sorted_rlx_labels_handles = sorted(
        [(label, handles[i]) for i, label in enumerate(labels) if "rlx" in label],
    ...
    non_rlx_labels_handles = [
        (label, handles[i]) for i, label in enumerate(labels) if "rlx" not in label
    ]
